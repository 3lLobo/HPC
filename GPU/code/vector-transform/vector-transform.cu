...
void vectorTransformCuda(int n, float* a, float* b, float* result) {
    ...
    // execute kernel
    kernelTime1.start();
    vectorTransformKernel<<<ceil((double)n/threadBlockSize), threadBlockSize>>>(deviceA, deviceB, deviceResult);
...
int main(int argc, char* argv[]) {
    ...
    int arr[5] = { 256, 1024, 65536, 655360, 1000000 };
